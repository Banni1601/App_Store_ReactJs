...
class App extends Component {
  state = { serachInput: "", list: appsList, activeTab: tabsList[0].tabId };
  ...
  getApps = (specificAppArr) => {
    const { serachInput, list, activeTab } = this.state;
    let arr = specificAppArr.filter((i) => i.category === activeTab);
    return arr;
  };

  getSpecificApps = () => {
    const { serachInput, activeTab, list } = this.state;
    let specificAppArr = list.filter((i) =>
      i.appName.toLowerCase().includes(serachInput.toLowerCase()),
    );

    return specificAppArr;
  };

  render() {
    const { serachInput, activeTab, list } = this.state;
    let specificAppArr = this.getSpecificApps();
    let arr = this.getApps(specificAppArr);
